...
if __name__ == "__main__":
    ...
    for it, batch in enumerate(tqdm(loader, bar_format="{desc:<5.5}{percentage:3.0f}%|{bar:50}{r_bar}")):
        ...
        if id_vote == args.num_votes:
            ...
            # sem_labels = label & 0xFFFF
            # res_sem_ = np.array([CATEGORY_TO_INDEX[NAME_MAPPING_K2A[labeldict[learning_map_inv[learning_map[x]]]]] for x in sem_labels], dtype=np.uint8)
            # res_sem_ = []
            # for l in label:
            #     if l in labeldict:
            #         res_sem_.append(CATEGORY_TO_INDEX[NAME_MAPPING_K2A[labeldict[l]]])
            #     else:
            #         res_sem_.append(0)
            ...
            with h5py.File(os.path.join(args.path_dataset, f'{scene_id}.h5'), 'r+') as f:    
                ...
                if 'flow_category_indices' not in f[key]:
                    print(f"Flow category indices not found in {key}. Skipping...")
                    continue
                valid_class = np.isin(f[key]['flow_category_indices'][:], valid_index_)
                gm = f[key]['ground_mask'][:]
                ego_mask = ~dataset.egopts_mask(f[key]['lidar'][:], min_bound=[-1.5, -1.5, -2.0], max_bound=[1.5, 1.5, 2.0])
                # ego_mask = ~dataset.egopts_mask(f[key]['lidar'][:])

                # res_sem = np.zeros_like(valid_class, dtype=np.uint8)
                res_sem = np.array(res_sem_, dtype=np.uint8)

                seg_valid = valid_class & ~gm & ~ego_mask
                # res_sem[~seg_valid] = 0
                ...
                if tmp_seg_name in f[key]:
                    ...
                    # continue
                f[key].create_dataset(tmp_seg_name, data=res_sem.astype(np.uint8))
                # print(f"wrote {tmp_seg_name} to {key} in scene {scene_id}.")
